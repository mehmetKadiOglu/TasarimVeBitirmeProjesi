...
function [bestSolution, bestFitness, iter]=SDO_Case_4_7_RFDB(fhd, dimension, maxIteration, fNumber)
    ...
    for Iter=1:MaxIt   
        ...
        for i=1:MarketSize 
            ...
            NewCommQuantity=CommQuantityEqu+Alpha.*( CommPrice(i,:)-CommPriceEqu); %Equation (13)   
             ...
             if rand<0.4
                NewCommPrice=CommQuantity(fdbIndex,:)-Beta.*(NewCommQuantity-CommQuantityEqu );% Equation (14)
             else
             ...
             if NewCommPriceFit<= CommPriceFit(i)
                 CommPriceFit(i)=NewCommPriceFit(i);
                 CommPrice(i,:)=NewCommPrice(i,:);
             end       
        end    
        ...
        for i=1:MarketSize        
            if  CommQuantityFit(i)<= CommPriceFit(i)
                CommPriceFit(i)= CommQuantityFit;
                CommPrice(i,:)= CommQuantity;
            end        
        end
