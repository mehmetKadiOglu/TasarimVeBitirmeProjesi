...
function [bestSolution, bestFitness, iter]=SDO_FDBP_Case_1_2(fhd, dimension, maxIteration, fNumber)
    ...
    for Iter=1:MaxIt   
        ...
        for i=1:MarketSize 
            ...
            Beta=2*cos((2*pi)*rand(1,Matr(Ind))); % Equation (17)
             ...
             if rand()<sigmoid
                [~, worstIndex] = max(CommPriceFit);
                [~, bestIndex] = min(CommPriceFit);
                if CommPriceFit(worstIndex) > NewCommQuantityFit
                    Populasyon(1).Fitness = CommPriceFit(i);
                    Populasyon(1).Dimension = CommPrice(i,:);

                    Populasyon(2).Fitness = NewCommPriceFit;
                    Populasyon(2).Dimension = NewCommPrice;

                    Populasyon(3).Fitness = CommPriceFit(worstIndex);
                    Populasyon(3).Dimension = CommPrice(worstIndex,:);

                    Populasyon(4).Fitness = CommPriceFit(bestIndex);
                    Populasyon(4).Dimension = CommPrice(bestIndex,:);
