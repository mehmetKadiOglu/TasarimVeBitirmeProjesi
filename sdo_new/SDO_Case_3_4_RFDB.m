...
function [bestSolution, bestFitness, iter]=SDO_Case_3_4_RFDB(fhd, dimension, maxIteration, fNumber)
    ...
    for Iter=1:MaxIt   
        ...
        for i=1:MarketSize 
            Ind=round(rand)+1;
            fdbIndex = rouletteFitnessDistanceBalance(CommPrice, CommPriceFit);
            ...
            Beta=2*cos((2*pi)*rand(1,Matr(Ind))); % Equation (17)
             ...
             if rand<0.9
                NewCommQuantity=CommQuantityEqu+Alpha.*( CommPrice(fdbIndex,:)-CommPriceEqu); %Equation (13)  
             else
