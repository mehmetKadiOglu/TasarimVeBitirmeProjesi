function [bestSolution, bestFitness, iter]=AGDE_FDB_Case_1_3(fhd, dimension, maxIteration, fNumber)
    ...
    for j = 1:NP % for each individual
         %%%%%%%%ADAPTIVE CR RULE  %%%%%%%%%%%%%%%%%%%%%%%%%
           ...
           paraIndex=floor (rand(1,1)*length(AA))+1;
                ...
                r(3) = CC(paraIndex2);
                F=0.1+0.9*rand(1,1);
                fdbIndex = rouletteFitnessDistanceBalance( Pop', Fit);
                Rnd = floor(rand()*D) + 1;
                for i = 1:D
                    if ( rand()<CR ) || ( Rnd==i )
                        if rand<w
                            X(i)=Pop(i,fdbIndex)+F*(Pop(i,r(1))-(Pop(i,r(2))));
                        else
