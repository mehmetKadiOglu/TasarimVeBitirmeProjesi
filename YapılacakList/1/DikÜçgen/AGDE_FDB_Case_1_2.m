function [bestSolution, bestFitness, iter]=AGDE_FDB_Case_1_2(fhd, dimension, maxIteration, fNumber)
...
for j = 1:NP % initialize each individual
    ...
    Fit(1,j)=testFunction(Pop(:,j), fhd, fNumber); 
end
[~, iBest]=min(Fit);
% ****************** %
% ** OPTIMIZATION ** %
    ...
    for j = 1:NP % for each individual
         %%%%%%%%ADAPTIVE CR RULE  %%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %%%%%%%%%%%%%%%%%END OF CR RULE%%%%%%%%%%%%%%%%%%%%%%%%%%%
                f=Fit;
                if rand() < 0.5
                    in = FDBIndexDinamik( Pop', Fit, ((g/GEN * -0.6) + 0.6) );
                else
           ...
           paraIndex=floor (rand(1,1)*length(AA))+1;
                ...
                F=0.1+0.9*rand(1,1);
                rouletFdbIndex = FDBRDinamik( Pop', Fit, ((g/GEN * -0.6) + 0.6) );
                Rnd = floor(rand()*D) + 1;
                for i = 1:D
                    if ( rand()<CR ) || ( Rnd==i )
