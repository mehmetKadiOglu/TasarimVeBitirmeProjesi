...
function [bestSolution, bestFitness, iter]=AEO_Case_2_27(fhd, dimension, maxIteration, fNumber)
...
for It=1:MaxIt    
    ...
    for i=3:nPop
     ...
     for i=1:nPop
         ...
         if rand <0.9
             fdbIndex = FDBRDinamik( PopPos, PopFit, ((It/MaxIt * -0.6) + 0.6) );
             newPopPos(i,:)=PopPos(fdbIndex,:)+3*randn(1,Matr(Ind)).*((r3*randi([1 2])-1)*PopPos(indOne,:)-(2*r3-1)*PopPos(i,:)); %equation (9)
         else
