function [bestSolution, bestFitness, iteration]=...
    efo(fhd, dimension, maxFe, lowerBand, upperBand, varargin)
...
maxval = upperBand;
Max_gen = maxFe;
N_emp = 50; % elektromanyetik parçacýk sayýsý
R_rate=0.3; %oluþturulmuþ bir parçacýðýn, rastgele bir parçacýkla yer deðiþtirme oraný
Ps_rate=0.2; % pozitif alanda üretilen parçacýðýn seçilme oraný
P_field=0.1; % pozitif alana ait nüfusun bir kýsmý.
N_field=0.45; % negatif alana ait nüfusun bir kýsmý.
%----------------------------------------------------------------------------------

phi = (1 + sqrt(5))/2;%altýn oran
%initializatin
...
while (generation <= Max_gen)
    ...
    % Senaryo 3, Senaryo 3 ile alakalý.
    ...
    if ( rp(1,generation) < R_rate)
        ...
        % ----------------- Rastgele Senaryo ----------------- %
        % Rastgele deðer yerine fdb'den gelen Ç.A.'nýn deðerini 
        % atýyoruz
        random_row_index = divDistIndex(em_pop, fit);
        random_value = em_pop(random_row_index, RI);
        new_emp(RI) = random_value;
        % ----------------- Rastgele Senaryo ----------------- %
        RI=RI+1;
        if (RI > N_var)
            RI=1;
