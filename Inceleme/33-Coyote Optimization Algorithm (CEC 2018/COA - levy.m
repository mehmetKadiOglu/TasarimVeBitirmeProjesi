function [GlobalParams,GlobalMin] = COA(F_index, VarMin,VarMax, nfevalMAX,n_packs,n_coy,dim)
global PAR
%% ------------------------------------------------------------------------
...
% [GlobalParams,GlobalMin] = COA_vPub(FOBJ, lu, nfevalMAX,Np,Nc);
%
...
for c=1:pop_total
    costs(c,1) = test_functions(coyotes(c,:),F_index);
end
nfeval = pop_total;
%% Output variables
...
while nfeval<nfevalMAX % Stopping criteria
    ...
    for p=1:n_packs
        ...
        for c=1:n_coy_aux
            ...
            new_cost = test_functions(new_coyotes(c,:),F_index);
            nfeval   = nfeval+1;
...
% C_V=std(coyotes_aux,0,1)./abs(mean(coyotes_aux));% sensing_distance using Eqs.(18) and (19)
...
% u=randn(D,length(mcol),2)*sigma;v=randn(D,length(mcol),2);step=u./abs(v).^(1/beta);L=abs(0.01*step);% Draw Levy flight sample

u=randn(1,D)*sigma;v=randn(1);step=u./abs(v).^(1/beta);L=100*abs(0.01*step);% Draw Levy flight sample
...
if(PAR.levy==1)
       r               = L(1:D-2);
        ...
        pup_cost    = test_functions(pup,F_index);
        nfeval      = nfeval + 1;
    ...
    [GlobalMin,ibest]   = min(costs);
    GlobalParams        = coyotes(ibest,:); 
    disp(['Iteration ' num2str(year) ': Best Cost = ' num2str(GlobalMin)]);
end
end
