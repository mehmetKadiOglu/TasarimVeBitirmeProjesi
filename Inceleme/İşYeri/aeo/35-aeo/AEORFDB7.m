...
function [bestSolution, bestFitness, iter]=AEORFDB7(fhd, dimension, maxIteration, fNumber)
...
for It=1:MaxIt    
    ...
    fdbIndex2 = fitnessDistanceBalance(PopPos, PopFit);
    for i=3:nPop
        u=randn(1,Dim);
        v=randn(1,Dim);  
        C=1/2*u./abs(v);  
        ...
        if r<1/3
            if rand<0.2
                ...
                newPopPos(i,:)=PopPos(i,:)+C.*(PopPos(fdbIndex2,:)-PopPos(fdbIndex,:)); %equation (6)
            else
