...
function [bestSolution, bestFitness, iter]=Case_1_10(fhd, dimension, maxIteration, fNumber)
...
for i=3:nPop
    ...
    if r<1/3
        ...
        if 1/3<r<2/3  
            if rand<0.4
                newPopPos(i,:)=PopPos(fdbIndex,:)+C.*(PopPos(i,:)- PopPos(randi([2 i-1]),:)); %equation (7)
            else
