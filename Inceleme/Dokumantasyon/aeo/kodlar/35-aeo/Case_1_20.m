...
function [bestSolution, bestFitness, iter]=Case_1_20(fhd, dimension, maxIteration, fNumber)
    ...
    for i=3:nPop
        ...
        if r<1/3
            ...
            if 1/3<r<2/3            
                ...
                r2=rand;  
                if rand<0.4
                    newPopPos(i,:)=PopPos(i,:)+C.*(r2.*(PopPos(i,:)- PopPos(fdbIndex,:))+(1-r2).*(PopPos(i,:)-PopPos(randi([2 i-1]),:))); %equation (8)
                else
