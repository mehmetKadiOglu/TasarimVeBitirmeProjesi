...
function [bestSolution, bestFitness, iter]=Case_1_4(fhd, dimension, maxIteration, fNumber)
...
for It=1:MaxIt    
    ...
    FdbPop = PopPos(fdbIndex,:)-newPopPos(1,:);
    for i=3:nPop
        ...
        if r<1/3
            if rand<0.3
                newPopPos(i,:)=PopPos(i,:)+C.*(FdbPop); %equation (6)
            else
