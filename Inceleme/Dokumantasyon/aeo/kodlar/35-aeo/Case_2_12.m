...
function [bestSolution, bestFitness, iter]=Case_2_12(fhd, dimension, maxIteration, fNumber)
    ...
    if r<1/3
        ...
        if 1/3<r<2/3
            if rand<0.2
                ...
                newPopPos(i,:)=PopPos(i,:)+C.*(PopPos(fdbIndex,:)- PopPos(randi([2 i-1]),:)); %equation (7)
            else
