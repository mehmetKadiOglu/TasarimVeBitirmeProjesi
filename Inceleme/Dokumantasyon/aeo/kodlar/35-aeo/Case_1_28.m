...
function [bestSolution, bestFitness, iter]=Case_1_28(fhd, dimension, maxIteration, fNumber)
...
for It=1:MaxIt    
    ...
    for i=3:nPop
     ...
     for i=1:nPop
         ...
         if rand <0.3
             newPopPos(i,:)=PopPos(indOne,:)+3*randn(1,Matr(Ind)).*((r3*randi([1 2])-1)*PopPos(fdbIndex,:)-(2*r3-1)*PopPos(i,:)); %equation (9)
         else
