...
public class ProxyFactory {
    ...
    private MyCustomProxy createProxy(Object bean) {
        ...
        return new MyCustomProxy(Arrays.asList(bean.getClass().getInterfaces()), proxyObj, bean.getClass().toString());
    }
    ...
    public <T> T getBean(Class<T> clazz) {
       /* Object proxy = beanRegistry.stream()
        ...
        MyCustomProxy myCustomProxy = beanRegistry.stream().filter(myCustomProxy1 -> myCustomProxy1.getClassType().equals(clazz.toString())).findFirst().get();

        //return clazz.cast(proxy);

        return (T) myCustomProxy.getInterfaces().get(0).cast(myCustomProxy.getJdkProxy());
    }
