...
function [bestSolution, bestFitness, iter]=MRFOCase_2_20(fhd, dimension, maxIteration, fNumber)
    settings;
    Low = lbArray;
    Up = ubArray;
    Dim = dimension;
            ...
            if rand<0.5
               ...
               Beta=2*exp(r1*((MaxIt-It+1)/MaxIt))*(sin(2*pi*r1));    
                 if  Coef>rand 
                    ...
                    Alpha=2*rand(1,Dim).*(-log(rand(1,Dim))).^0.5;   
                    newPopPos(i,:)=PopPos(i,:)+rand(1,Dim).*(PopPos(fdbIndex,:)-PopPos(i,:))+Alpha.*(BestX-PopPos(i,:)); %Equation (1) 
           end         
        end
