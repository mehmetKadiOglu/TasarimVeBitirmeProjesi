...
function [bestSolution, bestFitness, iter]=MRFOCase_1_25(fhd, dimension, maxIteration, fNumber)
settings;
Low = lbArray;
Up = ubArray;
Dim = dimension;
    ...
    for i=2:nPop
        if rand<0.5
           ...
           Beta=2*exp(r1*((MaxIt-It+1)/MaxIt))*(sin(2*pi*r1));    
            ...
            Alpha=2*rand(1,Dim).*(-log(rand(1,Dim))).^0.5;   
            if rand<0.5
                newPopPos(i,:)=PopPos(i,:)+rand(1,Dim).*(PopPos(i-1,:)-PopPos(i,:))+Alpha.*(BestX-PopPos(fdbIndex,:)); %Equation (1)
            else
