...
function [bestSolution, bestFitness, iter]=MRFOCase_1_29(fhd, dimension, maxIteration, fNumber)
settings;
Low = lbArray;
Up = ubArray;
Dim = dimension;
...
for It=1:MaxIt  
    ...
    for i=2:nPop
        ...
        for i=1:nPop 
            if rand<0.5
                newPopPos(i,:)=PopPos(i,:)+S*(rand*PopPos(fdbIndex,:)-rand*PopPos(i,:)); %Equation (8)
            else
