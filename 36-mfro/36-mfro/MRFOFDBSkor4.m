...
function [bestSolution, bestFitness, iter]=MRFOFDBSkor4(fhd, dimension, maxIteration, fNumber)
settings;
Low = lbArray;
Up = ubArray;
Dim = dimension;
...
for It=1:MaxIt  
    ...
    for i=2:nPop
        ...
        for i=1:nPop   
            if rand<0.8
                newPopPos(i,:)=PopPos(i,:)+S*(FDBSkorBest(PopPos, PopFit, i)*BestX-rand*PopPos(i,:)); %Equation (8)
            else
