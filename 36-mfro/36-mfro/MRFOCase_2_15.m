...
function [bestSolution, bestFitness, iter]=MRFOCase_2_15(fhd, dimension, maxIteration, fNumber)
settings;
Low = lbArray;
Up = ubArray;
Dim = dimension;
                ...
                newPopPos(i,:)=BestX+rand(1,Dim).*(PopPos(i-1,:)-PopPos(i,:))+Beta*(BestX-PopPos(i,:)); %Equation (4)                               
                 ...
                 if rand<0.5
                  fdbIndex = rouletteFitnessDistanceBalance(PopPos, PopFit);
                    newPopPos(i,:)=IndivRand+rand(1,Dim).*(PopPos(i-1,:)-PopPos(i,:))+Beta*(IndivRand-PopPos(fdbIndex,:));  %Equation (7) 
                 else
