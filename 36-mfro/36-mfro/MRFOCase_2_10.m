...
function [bestSolution, bestFitness, iter]=MRFOCase_2_10(fhd, dimension, maxIteration, fNumber)
settings;
Low = lbArray;
Up = ubArray;
Dim = dimension;
...
for It=1:MaxIt  
    ...
    for i=2:nPop
        ...
        if rand<0.5
           ...
           Beta=2*exp(r1*((MaxIt-It+1)/MaxIt))*(sin(2*pi*r1));    
             if  Coef>rand 
                    newPopPos(i,:)=BestX+rand(1,Dim).*(PopPos(i-1,:)-PopPos(i,:))+Beta*(BestX-PopPos(fdbIndex,:)); %Equation (4)                                                    
             else
                 IndivRand=rand(1,Dim).*(Up-Low)+Low;                                
