...
function [bestSolution, bestFitness, iter]=MRFOCase_2_17(fhd, dimension, maxIteration, fNumber)
settings;
Low = lbArray;
Up = ubArray;
Dim = dimension;
        ...
        if rand<0.5
           ...
           Beta=2*exp(r1*((MaxIt-It+1)/MaxIt))*(sin(2*pi*r1));    
            ...
            if rand<0.5
                ...
                newPopPos(i,:)=PopPos(fdbIndex,:)+rand(1,Dim).*(PopPos(i-1,:)-PopPos(i,:))+Alpha.*(BestX-PopPos(i,:)); %Equation (1)
            else
